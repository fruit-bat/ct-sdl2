...
static void X11_GetWindowRootPos(Display *display, Window window, int *x, int *y) {
    
    XWindowAttributes xwa;
    
    X11_XGetWindowAttributes(display, window, &xwa);
    printf("Found window x,y  %d, %d\n", xwa.x, xwa.y);
    
    Window cr;
    X11_XTranslateCoordinates(
      display,
      window,
      xwa.root, 
      0, //xwa.x,  Why does 0 work??
      0, //xwa.y,  Why does 0 work??
      x,
      y,
      &cr); // Child return
    
    printf("Found window root x,y  %d, %d\n", *x, *y);
}
...
#ifdef DEBUG_XEVENTS
            printf("window %p mode %p detail %p: FocusOut!\n", xevent.xfocus.window, xevent.xfocus.mode, xevent.xfocus.detail);
#endif

            Window fw, rw;
            X11_XGetInputFocus(display, &fw, &rw); 
printf("focus window %p, revert window %p\n", fw, rw);

            // BUG in X11 linaro??
            if(fw == xevent.xfocus.window) {
              break;
            }
    ...
    printf("Found window border %ld, %ld, %ld, %ld\n", border_left, border_right, border_top, border_bottom);
                    
                }
            }

            SDL_Window *sdlWindow = data->window;
            int fullscreen_mode = sdlWindow->flags & SDL_WINDOW_FULLSCREEN;
            int tx,ty;
            X11_GetWindowRootPos(display, data->xwindow, &tx, &ty);

            xevent.xconfigure.x = tx - border_left;
            xevent.xconfigure.y = ty - border_top;
            ...
            if(bordered) {
              sdlWindow->flags &=~SDL_WINDOW_BORDERLESS;
            }
            else {
              sdlWindow->flags |= SDL_WINDOW_BORDERLESS;              
            }
            
    printf("Found window actual m,x,y %d,%d, %d\n", fullscreen_mode, xevent.xconfigure.x, xevent.xconfigure.y);
